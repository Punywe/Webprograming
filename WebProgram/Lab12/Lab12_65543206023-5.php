...
<head>
    ...
    <script>
     ...
     // Separate CheckNum function
     function CheckNum(field) {
        const value = field.value;

        if (!/^\d*$/.test(value)) {
            alert("กรุณากรอกเฉพาะตัวเลขเท่านั้น");
            field.value = ""; // Clear the field if not valid
        }
    }

    function CheckNameAndAdd(field) {
    ...
    function CheckNumPost(field) {
    const value = field.value.trim();
    if (value && value.length !== 5) {
        alert("กรุณากรอกข้อมูลให้ครบ 5 ตัวอักษร!!!");
        field.value = ""; // Clear the field if not valid
    }
    }

    function CheckNumPhone(field) {
    const value = field.value.trim();
    if (value && value.length !== 10) {
        alert("กรุณากรอกข้อมูลให้ครบ 10 ตัวอักษร!!!");
        field.value = ""; // Clear the field if not valid
    }
    }
    ...
    function CheckPass(field) {
        if (field.value.trim().length < 8) {
            alert("กรุณากรอกข้อมูลให้ครบ 8 ตัวอักษร!!!");
            field.value = "";
        }
...
<body>
    ...
    <legend>ข้อมูลลูกค้า</legend>
        <label for="fname">ชื่อ:</label>
            <input type="text" id="fname" name="Fname" placeholder="ใส่ชื่อจริง" onblur="CheckNameAndAdd(this)"><br><br>

        <label for="lname">นามสกุล:</label>
            <input type="text" id="lname" name="lname" placeholder="ใส่นามสกุล" onblur="CheckNameAndAdd(this)" ><br><br>
        ...
        <input type="text" id="add" name="add" placeholder="ใส่ที่อยู่ปัจจุบัน" onblur="CheckNameAndAdd(this)" ><br><br>
        ...
        <input type="text" id="pnum" name="pnum" placeholder="ใส่เลขไปรษณีย์" onblur="CheckNumPost(this)" onkeyup="CheckNum(this)" ><br><br>

        <label for="phone">โทรศัพท์:</label>
        <input type="text" id="phone" name="phone" placeholder="ใส่เบอร์โทรศัพท์ของคุณ"  onblur="CheckNumPhone(this)" onkeyup="CheckNum(this)" ><br><br>
        ...
        <textarea id="info" name="info" rows="5" cols="50" placeholder="โปรดใส่รายละเอียดเพิ่มเติมเกี่ยวกับตัวคุณ"></textarea><br><br>
            ...
            <fieldset>
                ...
                <label for="username">Username:</label>
                    <input type="text" id="username" name="username" placeholder="" onblur="CheckUser(this)"><br><br>

                 <label for="pwd">Password:</label>
                    <input type="password" id="pwd" name="pwd" placeholder="" onblur="CheckPass(this)"><br><br>     
                    
                <label for="Cpwd">Confirm Password:</label>
                    <input type="password" id="Cpwd" name="Cpwd" placeholder="" onblur="CheckPass(this)"><br><br>   
            </fieldset>
        </fieldset>      
