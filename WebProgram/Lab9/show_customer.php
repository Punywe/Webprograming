...
if (!isset($_SESSION["User"])) {
    ...
    exit(); // ถ้ายังไม่เข้าสู่ระบบ ให้เปลี่ยนเส้นทางไปยังหน้า login
}

// แสดงชื่อจริงและนามสกุล
$userName = isset($_SESSION["UserName"]) ? $_SESSION["UserName"] : "ไม่ระบุชื่อ";
$userLastname = isset($_SESSION["UserLastname"]) ? $_SESSION["UserLastname"] : "ไม่ระบุชื่อ";

// แสดงข้อความต้อนรับ
echo "ยินดีต้อนรับ: $userName $userLastname";
echo "<p style='text-align: center;'>ข้อมูลลูกค้า</p>";
echo "<p style='text-align: right; color: red;'><a href='add_customer.php'><img src='03.png' alt='แก้ไข' style='width:16px; height:16px;'>เพิ่มข้อมูลลูกค้า</a></p>";
...
$sql = "SELECT Customer_id, Customer_Name, Customer_Lastname, Province, Telephone FROM customer WHERE status = 0";
...
if (mysqli_num_rows($result) > 0) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // แก้ไขและลบลิงก์
        echo "<td style='border: 1px solid black; padding: 10px;'><a href='update_customer.php?id=".$row['Customer_id']."'><img src='02.png' alt='แก้ไข' style='width:16px; height:16px;'></a></td>";
        echo "<td style='border: 1px solid black; padding: 10px;'><a href='show_customer.php?id=".$row['Customer_id']."'onclick='return confirm(\"คุณแน่ใจหรือไม่ว่าต้องการลบลูกค้าคนนี้?\");'><img src='01.png' alt='ลบ' style='width:16px; height:16px;'></a></td>";
        echo "</tr>";
    }
    echo "</table>";
